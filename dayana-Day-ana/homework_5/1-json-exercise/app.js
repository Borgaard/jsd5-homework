...
var parsed = JSON.parse(grungeAlbumsJSON);
// console.log(parsed);
// console.log(parsed == grungeAlbums); //False they are not the same properties have been alphabatized
...
//  Units sold: 31234
    ...
    for (var key in parsed.albums){

        albumInfo += "Album: " + parsed.albums[key].name + "\n";
        albumInfo += "Artist: " + parsed.albums[key].artist+ "\n";
        albumInfo += "Units Sold: " + parsed.albums[key].unitsSold;
...
var ballersStringed = JSON.stringify(ballers);
console.log(ballersStringed);
var ballersParsed = JSON.parse(ballersStringed);
console.log(ballersParsed);
console.log(ballers == ballersParsed);

//has not changed according to the original but they do not equal each other
